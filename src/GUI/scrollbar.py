...
class ScrollBarBase(base.GUIBase):
    def __init__(self, u, v, item_width, item_height, field_width, field_height,
                 bar_style: base.ScrollBarStyle = profile.default_scrollbar_style,
                 padding = profile.default_padding, no_register=False) -> None:
    ...
    def draw(self, surface: Optional[_pg.Surface] = None):
        ...
        if not self.bar_style.rounded:
            _pg.draw.line(surface, self.bar_style.color, 
                            (self.start_pos*profile.surface_scale).pos, (self.end_pos*profile.surface_scale).pos, self.bar_style.width)
        else:
            # 補正
            ...
            _pg.draw.circle(surface, self.bar_style.color, # start edge
                            (self.start_pos*profile.surface_scale).pos, self.bar_style.width/2)
            _pg.draw.circle(surface, self.bar_style.color, # end edge
                            (self.end_pos*profile.surface_scale).pos, self.bar_style.width/2)
        self._flush(surface)
