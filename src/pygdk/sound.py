...
class AudioManager:
    ...
    def stop_bgm():
        if AudioManager.playing_bgm_index == -1:
            print("[AudioManager.stop_bgm] No BGM is played yet.")
            return
        AudioManager.bgm_stack[AudioManager.playing_bgm_index][1].stop()
    ...
    def set_se_volume(value: float):
        ...
        """
        if value > 1:
            print(f"[AudioManager.set_se_volume] invalid value: {value} > 1")
            return
    ...
    def set_bgm_volume(value: float):
        ...
        """
        if value > 1:
            print(f"[AudioManager.set_bgm_volume] invalid value: {value} > 1")
            return
